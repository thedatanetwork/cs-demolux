          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            ...
            <div className="text-center lg:text-left">
              ...
              <div className="inline-flex items-center space-x-2 bg-white/80 backdrop-blur-sm rounded-full px-4 py-2 mb-6">
                ...
                <span className="text-sm font-medium text-gray-900">
                  {heroData?.hero_badge_text || 'Premium Luxury Technology'}
                </span>
              </div>

              {/* Main Headline */}
              <h1 className="font-heading text-4xl md:text-5xl lg:text-6xl font-bold text-gray-900 leading-tight mb-6">
                {heroData?.hero_subtitle || 'Where Innovation'}
                <br />
                <span className="text-gradient bg-gradient-to-r from-gold-600 to-gold-400">
                  {heroData?.hero_title?.replace(heroData.hero_subtitle, '').trim() || 'Meets Luxury'}
                </span>
              </h1>

              {/* Subheading */}
              <p className="text-lg md:text-xl text-gray-700 mb-8 max-w-2xl">
                {heroData?.hero_description || 'Discover the future with Demolux premium wearable technology and innovative technofurniture. Each piece is crafted where cutting-edge design meets exceptional performance.'}
              </p>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
                {(heroData?.hero_features || [
                  { icon: 'Award', title: 'Premium Quality', description: 'Exceptional craftsmanship' },
                  { icon: 'Zap', title: 'Innovation', description: 'Cutting-edge technology' },
                  { icon: 'Star', title: 'Luxury Design', description: 'Sophisticated aesthetics' }
            ...
            <div className="relative">
              <div className="relative z-10">
                {(() => {
                  // Handle both array and single object formats for hero_image
                  const heroImage = Array.isArray(heroData?.hero_image) 
                    ? heroData.hero_image[0] 
                    : heroData?.hero_image;
                  
                  return heroImage?.url ? (
                    /* Hero image from Contentstack */
